<template>
  <Dialog :style="{ width: '450px' }" header="Confirm" :modal="true">
    <div class="flex align-items-center justify-content-center">
      ...
      />
      <span v-if="formData"
        >Are you sure you want to delete the selected electricity billing payment?</span
      >
    </div>
    <template #footer>
      <Button
...
export default {
  ...
  methods: {
    async submit() {
      ...
      await this.$store.dispatch("electricityBillingPaymentModule/deleteDatas", this.formData);
