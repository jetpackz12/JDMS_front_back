<template>
  <Dialog
    header="Edit Electricity Payment"
    :style="{ width: '450px' }"
    :modal="true"
    class="p-fluid"
  >
...
export default {
  ...
  methods: {
    async submit() {
      ...
      this.formData.due_date = this.isValidDate(this.formData.due_date)
        ? this.formData.due_date.toLocaleDateString("en-CA")
        : this.formattedDate(this.formData.due_date);
      this.formData.date_issue = this.isValidDate(this.formData.date_issue)
        ? this.formData.date_issue.toLocaleDateString("en-CA")
        : this.formattedDate(this.formData.date_issue);

      await this.$store.dispatch(
        "electricityBillingPaymentModule/updateData",
        this.formData
      );
